...
result_dict = {}
info_dict_template = {'steps': {'setup': {'result': False, 'longrepr': None},
                                'call': {'result': False, 'longrepr': None},
                                'teardown': {'result': False, 'longrepr': None}
                                },
                      'result': '',
                      'longrepr': ''
                      }
...
def send_result2pms(script_path, info_dict):
    ...
    with open(os.path.join(constant.root_path, 'resource', 'pms', 'pms_job.json'), 'r', encoding='utf-8') as f, open(
            os.path.join(constant.root_path, 'resource', 'pms', 'name2id.json'), 'r', encoding='utf-8') as f2:
...
def send_all_result2pms():
    with open(os.path.join(constant.root_path, 'resource', 'pms', 'pms_job.json'), 'r', encoding='utf-8') as f:
        base_info = json.load(f)
