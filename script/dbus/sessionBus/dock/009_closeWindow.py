...
# ***************************************************
# @Test Case ID:          009_closeWindow
# @Test Description:      CloseWindow(win uint32),关闭给定id的窗口实例
#                         win: 请参考ActivateWindow函数的win参数
# @Test Condition:        1.打开一个文件管理器窗口
#                         2.获取文件管理器的窗口id
# @Test Step:             1.调用 CloseWindow 函数,传入文件管理器的窗口id
# @Test Result:           1.程序无报错
# @Test Remark:
# @Author:  ut001627
# ***************************************************
...
class TestCase(OSBase):
    ...
    @pytest.mark.public
    def test_step(self):
        self.Step("步骤1:调用 CloseWindow 函数,传入文件管理器的窗口id")
        dock.closeWindow(self.win_id)
