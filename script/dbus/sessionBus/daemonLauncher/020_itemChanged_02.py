...
# ***************************************************
# @Test Case ID:          020_itemChanged_02
...
# @Test Result:           4.监控到ItemChanged信号,'string "updated"'和'string "vlc"'在返回值中
# @Test Remark:
# @Author:  ut001627
# ***************************************************
...
class TestCase(OSBase):
    ...
    def setUp(self):
        ...
        self.reinstall_command = f'apt-get -y --reinstall install {self.app}'
        self.remove_command = f'sudo apt-get -y remove {self.app}'
        self.single = 'ItemChanged'
        self.parse_flgs = ('string "updated"', 'string "vlc"')
    ...
    def test_step(self):
        ...
        logging.info(root_execute_command_by_stdin(self.reinstall_command, self.passwd))
        time.sleep(2)

        self.Step(
            f'''setp04:解析dbus-monitor结果,是否监控到ItemChanged信号,判断'string "updated"'和'string "vlc"'是否在返回值中''')
