...
from aw.dbus.sessionBus.daemonLauncher import isItemOnDesktop, requestRemoveFromDesktop
...
class TestCase(OSBase):
    ...
    @pytest.mark.public
    def test_step(self):
        self.Step(f'步骤2:检查{self.app_id}桌面图标是否存在')
        assert isItemOnDesktop(app_id=self.app_id, target=False)
        time.sleep(2)
