...
# ***************************************************
# @Test Case ID:          001_addAutostart_01
# @Test Description:      AddAutostart (string filename) -> (bool value),设置文件自动启动
# @Test Condition:        1.无
# @Test Step:             1.调用 AddAutostart 方法添加一个合法并存在的文件（/usr/share/applications
...
class TestCase(OSBase):
    ...
    @pytest.mark.public
    def test_step(self):
        self.Step("步骤1:调用 AddAutostart 方法添加一个合法并存在的文件")
        sessionManagerStartManager.addAutostart(self.file)
