...
#                         2.调用 GenerateLocale 函数,传入错误的语言信息
#                         3.解析dbus-monitor结果,是否监控到 Success 信号,判断 'boolean false' 是否在返回值中
...
class TestCase(OSBase):
    ...
    @pytest.mark.public
    def test_step(self):
        self.Step("步骤2:调用 GenerateLocale 函数,传入错误的语言信息")
        apiLocaleHelper.generateLocale(locale='test', target='error', passwd=self.passwd)
        time.sleep(5)

        self.Step("步骤3:解析dbus-monitor结果,是否监控到Success信号,判断'boolean false'是否在返回值中")
